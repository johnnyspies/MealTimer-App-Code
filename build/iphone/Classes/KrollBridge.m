...
NSString * MealtimerNoXML$ModuleRequireFormat = @"(function(exports){"
...
@implementation MealtimerNoXMLObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_mealtimernoxml);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create MealtimerNoXML global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "MealtimerNoXML" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_mealtimernoxml = [[MealtimerNoXMLObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_mealtimernoxml];
	
	NSString *_mealtimernoxmlNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _mealtimernoxmlNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_mealtimernoxml valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:MealtimerNoXML$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.mealtimernoxml.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.mealtimernoxml.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
