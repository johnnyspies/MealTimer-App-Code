...
//Rare exceptions to indicate a bug in the _mealtimernoxml code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_mealtimernoxml = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_mealtimernoxml]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.mealtimernoxml.module"
